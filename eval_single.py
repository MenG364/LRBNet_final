...
@torch.no_grad()
def evaluate_by_single(model, dataloader, args, device):
    model.eval()
    ...
    return score1 / num_examples, score2 / num_examples, score3 / num_examples, score4 / num_examples
...
def evaluate(model, eval_loader, args, device=torch.device("cuda")):
    ...
    scores = evaluate_by_single(model, eval_loader, args, device)
    logger = utils.Logger(os.path.join('eval', 'log.txt'))
    logger.write('score1=%7.3f,score2=%7.3f,score3=%7.3f,score4=%7.3f' % (scores[0]*100, scores[0]*100, scores[0]*100, scores[0]*100))
