...
from LR_models.question_hierarchy_rnn import question_hierarchy_rnn
...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        emb = self.emb(x)
        if 'c' in self.op:
            emb = torch.cat((emb, self.emb_(x)), 2)  # [batch_size, seq_length, 600]
...
class TextEmbedding(nn.Module):
    def __init__(self, in_dim, num_hid, nlayers, bidirect, dropout,
        ...
        self.rnn = question_hierarchy_rnn(in_dim, num_hid, nlayers, bidirect, dropout, rnn_type=rnn_type)
    ...
    def forward(self, x, lens):
        ...
        batch = x.size(0)
        hidden = self.init_hidden(batch)
        output, hidden = self.rnn(x, hidden, lens)
    ...
    def forward_all(self, x, lens):
        # x: [batch, sequence, in_dim]
        batch = x.size(0)
        hidden = self.init_hidden(batch)
        output, _ = self.rnn(x, hidden, lens)
        return output
...
class TextSelfAttention(nn.Module):
    ...
    def forward(self, ques_feat):
        ...
        return ques_feat_self_att  # (batch, num_hid).
