...
class ImplicitRelationEncoder(nn.Module):
    def __init__(self, v_dim, q_dim, out_dim, dir_num, pos_emb_dim,
        ...
        if self.separation:
            self.film_L = FiLMGen(output_batchnorm=True,
                                  bidirectional=False,
                                  encoder_type='FCNet',
                                  act_type='linear',
                                  gamma_option='linear',
            ...
            self.film_R = FiLMGen(output_batchnorm=True,
                                  bidirectional=False,
                                  encoder_type='FCNet',
                                  act_type='linear',
                                  gamma_option='linear',
            ...
            self.film = FiLMGen(output_batchnorm=True,
                                bidirectional=False,
                                encoder_type='FCNet',
                                act_type='linear',
                                gamma_option='linear',
    ...
    def forward(self, v, q, cap, b, v_mask, cap_mask, position_embedding_L, position_embedding_R):
        ...
        for i in range(self.num_steps):
            ...
            ################# 添加注意力机制 #################
            # d_k = imp_cap_rel.size(-1)
            # scores = torch.matmul(
            #     imp_cap_rel, imp_v_rel.transpose(-2, -1)
            # ) / math.sqrt(d_k)
            # new_imp_v_rel=torch.matmul(F.dropout(F.softmax(scores,dim=-1)),imp_v_rel)
            # new_imp_cap_rel=torch.matmul(F.dropout(F.softmax(scores.transpose(-2, -1),dim=-1)),imp_cap_rel)
            ##############################################

            if self.separation:
                L_film = self.film_L(imp_cap_rel)  # [batch, num_rois, out_dim * 2]
                R_film = self.film_R(imp_v_rel)  # [batch, num_rois, out_dim * 2]
            else:
