...
class Counter(nn.Module):
    ...
    Count the number of objects from a set of bounding boxes
    and a set of scores for each bounding box.
    ...
    def forward(self, boxes, attention):
        """
        Forward propagation of attention weights
        and bounding boxes to produce count features.
        `boxes` has to be a tensor of shape (n, 4, m)
        with the 4 channels containing the x and y coordinates
        of the top left corner and the x and y coordinates
        of the bottom right corner in this order.
        `attention` has to be a tensor of shape (n, m).
        Each value should be in [0, 1] if already_sigmoided is set to True,
        but there are no restrictions if already_sigmoided is set to False.
        This value should be close to 1
        if the corresponding boundign box is relevant
        and close to 0 if it is not.
        ...
        # aggregate the score
        # can skip putting this on the diagonal
        # since we're just summing over it anyway
    ...
    def to_one_hot(self, scores):
        ...
        """
        # sanity check, I don't think this ever does anything
        ...
        target_r.scatter_(dim=1, index=(i + 1).clamp(max=self.objects),
                          value=1)
    ...
    def filter_most_important(self, n, boxes, attention):
        ...
        attention, idx = attention.topk(n if n<attention.size(1) else attention.size(1), dim=1, sorted=False)
