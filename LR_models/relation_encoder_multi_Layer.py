...
class ImplicitRelationEncoder(nn.Module):
    def __init__(self, v_dim, q_dim, out_dim, dir_num, pos_emb_dim,
        ...
        self.cap_transform = FCNet([in_dim, out_dim], act="LeakyReLU")
        ...
        self.implicit_relation_L = nn.ModuleList([GAT(dir_num, 1, out_dim, out_dim, nongt_dim=nongt_dim,
                                                      label_bias=label_bias, num_heads=num_heads,
                                                      pos_emb_dim=pos_emb_dim) for _ in range(num_steps)])
        self.implicit_relation_R = nn.ModuleList([GAT(dir_num, 1, out_dim, out_dim,
                                                      nongt_dim=nongt_dim,
    ...
    def forward(self, v, q, cap, b, v_mask, cap_mask, position_embedding_L, position_embedding_R):
        ...
        cap_q = self.cap_transform(cap_cat_q_bn)
        ...
        for i in range(self.num_steps):
            ...
            imp_cap_rel = self.implicit_relation_L[i].forward(cap_q,
                                                              imp_adj_mat_L,
                                                              position_embedding_L)
            imp_v_rel = self.implicit_relation_R[i].forward(v_cat_q_bn,
                                                            imp_adj_mat_R,
                                                            position_embedding_R)

            L_film = self.film_L[i](imp_cap_rel)  # [batch, num_rois, out_dim * 2]
            R_film = self.film_R[i](imp_v_rel)  # [batch, num_rois, out_dim * 2]

            new_imp_v_rel = F.relu(torch.mul(imp_v_rel, L_film[0]) + L_film[1])
            new_imp_cap_rel = F.relu(torch.mul(imp_cap_rel, R_film[0]) + R_film[1])

            if self.residual_connection:
                v_cat_q_bn = v_red + new_imp_v_rel
                cap_q = cap_red + new_imp_cap_rel
            else:
                v_cat_q_bn = new_imp_v_rel
                cap_q = new_imp_cap_rel

        return v_cat_q_bn, cap_q, count
