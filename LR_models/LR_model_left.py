...
from LR_models.relation_encoder_left import ExplicitRelationEncoder, ImplicitRelationEncoder
from ReGAT_models.fc import FCNet


class LR_model(nn.Module):
    def __init__(self, dataset, w_emb, q_emb, q_att, cap_emb, cap_att, v_relation,
                  left_classifier, joint_L, glimpse,
    ...
    def forward(self, v, c, b, q, implicit_pos_emb_L, implicit_pos_emb_R, sem_adj_matrix_L=None,
        ...
        if self.relation_type == "semantic":
            ...
            # v = torch.cat((v, b), -1)
            cap_emb, count = self.v_relation.forward(v, q_emb_self_att, cap_emb_self_att, b, v_mask,
                                                            cap_mask,
                                                            implicit_pos_emb_L, implicit_pos_emb_R)
            if self.use_count:
                count = self.count_proj(count)
        if self.left_classifier:
            cap_emb, att = self.joint_L(cap_emb, q_emb_seq, b)
            if self.use_count:
                cap_emb = cap_emb + count
            cap_logits = self.left_classifier(cap_emb)

        out = {'logits': (None, cap_logits, None), 'q_emb': q_emb_self_att, 'att': att}
...
def build_LR_model_left(dataset, args):
    ...
    gamma = 0
    return LR_model(dataset, w_emb, q_emb, q_att, cap_emb, cap_att, v_relation,
                    left_classifier, joint_L, gamma, args.fusion,
