    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        with tqdm(total=len(train_loader), ncols=160) as pbar:
            for i, (v, norm_bb, q, target, cap, _, _, _, bb) in enumerate(train_loader):
                ...
                # 3loss
                joint_logits= pred['logits']
                cap_logits=joint_logits
                v_logits=joint_logits
                if args.model == 'model' or args.model == 'left_right_model':
                    if (args.dataset == 'vqa' or args.dataset == 'vqa1.0'):
                        joint_loss = instance_bce_with_logits(joint_logits, target)
                        cap_loss = joint_loss #instance_bce_with_logits(cap_logits, target)
                        v_loss = joint_loss # instance_bce_with_logits(v_logits, target)
                        pred1 = torch.sigmoid(joint_logits) #(torch.sigmoid(joint_logits) + torch.sigmoid(cap_logits) + torch.sigmoid(v_logits)) / 3
                    else:
                    ...
                    loss = joint_loss #(joint_loss + cap_loss + v_loss) / 3
                    pred = joint_logits # (joint_logits + cap_logits + v_logits) / 3
