...
def tsv_writer1(idx, all_values, tsv_file, sep='\t'):
    pre_file = tsv_file
    pre_lineidx_file = op.splitext(pre_file)[0] + '.lineidx'
    mkdir(op.dirname(pre_file))

    feat_file = op.splitext(tsv_file)[0] + '_feat.tsv'
    feat_lineidx_file = op.splitext(feat_file)[0] + '.lineidx'
    mkdir(op.dirname(feat_file))

    with open(feat_file, 'a') as fp, open(feat_lineidx_file, 'a') as fpidx, \
            open(pre_file, 'a') as pp, open(pre_lineidx_file, 'a') as ppidx:
