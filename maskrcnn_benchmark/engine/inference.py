...
from maskrcnn_benchmark.structures.tsv_file_ops import tsv_writer, tsv_writer1
...
def compute_on_dataset1(cfg, model, data_loader, device, bbox_aug, dataset, output_folder, labelmap_file, timer=None):
    model.eval()
    results_dict = {}
    cpu_device = torch.device("cpu")
    idx=[0,0]
    for itr, batch in enumerate(tqdm(data_loader)):
        ...
        if itr % 500 == 0:
            predictions = _accumulate_predictions_from_multiple_gpus(results_dict, cfg.TEST.GATHER_ON_CPU)
            if not is_main_process():
                return

            output_tsv_name = 'predictions.tsv'
            idx=convert_predictions_to_tsv(
                idx,
...
def convert_predictions_to_tsv(idx,predictions, dataset, output_folder,
    ...
    def gen_rows():
        for idx, prediction in sorted(predictions.items()):
            image_key = dataset.get_img_key(idx)[-12:]
            image_key = str(int(image_key))
            ...
            if "attr_labels" in data_subset and "attr_scores" in data_subset:
                attr_labels = prediction.get_field("attr_labels")
                attr_scores = prediction.get_field("attr_scores")
                attr_labels = [attr_label[attr_conf > 0.01].tolist() for attr_conf, attr_label in
                               zip(attr_scores, attr_labels)]
                attr_scores = [attr_conf[attr_conf > 0.01].tolist() for attr_conf in attr_scores]
            ...
            if 'relation_scores_all' in data_subset:
                relation_scores_all = [base64.b64encode(relation_scores_all[i]).decode('utf-8') for i in
                                       range(len(relations))]
            ...
            for i in range(len(boxes)):
                ...
                for name in data_subset:
                    ...
                    if name == 'feature':
                        ...
                        #     .decode('utf-8')
                        cur_f['feature'] = base64.b64encode(features[i]) \
                            .decode('utf-8')
                    if name == 'scores_all':
            ...
            yield [image_key, json.dumps(
                {'image_h': image_height, 'image_w': image_width, 'objects': objects, 'relations': triplets})], [
                      image_key, len(boxes), features]

    idx=tsv_writer1(idx,gen_rows(), os.path.join(output_folder, output_tsv_name))
    return idx
...
def inference1(
    ...
    inference_timer = Timer()
    total_timer.tic()
    compute_on_dataset1(cfg, model, data_loader, device, bbox_aug, dataset, output_folder, labelmap_file,
                                      inference_timer)
    # wait for all processes to complete before measuring the time
    synchronize()
    total_time = total_timer.toc()
