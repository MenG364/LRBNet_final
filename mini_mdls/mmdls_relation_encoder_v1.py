...
class ImplicitRelationEncoder(nn.Module):
    def __init__(self, v_dim, q_dim, out_dim, dir_num, pos_emb_dim,
        ...
        self.vum = nn.ModuleList([VUM(out_dim, out_dim) for _ in range(num_steps)])

        self.film_L = nn.ModuleList([FiLMGen(output_batchnorm=True,
                                             bidirectional=False,
                                             encoder_type='FCNet',
                                             act_type='linear',
                                             gamma_option='linear',
                                             gamma_baseline=1,
                                             module_dim=out_dim) for _ in range(num_steps)])
        self.dropout1 = nn.ModuleList([nn.Dropout(0.1) for _ in range(num_steps)])
        self.norm1 = nn.ModuleList([LayerNorm(out_dim) for _ in range(num_steps)])
        self.relu1 = nn.ModuleList([nn.ReLU(inplace=True) for _ in range(num_steps)])

        self.dropout2 = nn.ModuleList([nn.Dropout(0.1) for _ in range(num_steps)])
        self.norm2 = nn.ModuleList([LayerNorm(out_dim) for _ in range(num_steps)])
        self.relu2 = nn.ModuleList([nn.ReLU(inplace=True) for _ in range(num_steps)])
        self.film_R = nn.ModuleList([FiLMGen(output_batchnorm=True,
                                             bidirectional=False,
                                             encoder_type='FCNet',
                                             act_type='linear',
                                             gamma_option='linear',
    ...
    def forward(self, v, q, cap, b, v_mask, cap_mask, position_embedding_L, position_embedding_R):
        ...
        imp_v_rel = imp_v.clone()

        for i in range(self.num_steps):
            imp_v_rel = self.vum[i](imp_v_rel, v_mask.unsqueeze(1).unsqueeze(2))
            imp_cap_rel = self.tum[i](imp_cap_rel, cap_mask.unsqueeze(1).unsqueeze(2))

            L_film = self.film_L[i](imp_v_rel)
            R_film = self.film_R[i](imp_cap_rel)
            imp_v_rel = self.norm1[i](imp_v_rel + self.dropout1[i](self.relu1[i](torch.mul(imp_v_rel, R_film[0]) + R_film[1])))
            imp_cap_rel = self.norm2[i](imp_cap_rel + self.dropout2[i](self.relu2[i](torch.mul(imp_cap_rel, L_film[0]) + L_film[1])))

        return imp_v_rel, imp_cap_rel, count
