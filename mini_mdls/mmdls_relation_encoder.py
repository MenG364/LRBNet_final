...
class ImplicitRelationEncoder(nn.Module):
    ...
    def forward(self, v, q, cap, b, v_mask, cap_mask, position_embedding_L, position_embedding_R):
        ...
        cap_cat_q_bn = self.T(cap_cat_q_bn)
        ...
        for i in range(self.num_steps):
            ...
            imp_cap_rel = self.tum[i](cap_cat_q_bn, cap_mask.unsqueeze(1).unsqueeze(2))

            imp_v_rel = self.vum[i](v_cat_q_bn, v_mask.unsqueeze(1).unsqueeze(2))
            ...
            if self.residual_connection:
                v_cat_q_bn = v_cat_q_bn + imp_v_rel
                cap_cat_q_bn = cap_cat_q_bn + imp_cap_rel
            else:
                v_cat_q_bn = imp_v_rel
                cap_cat_q_bn = imp_cap_rel

        return v_cat_q_bn, cap_cat_q_bn, count
