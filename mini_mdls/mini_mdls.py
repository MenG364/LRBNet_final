...
# from LR_models.relation_encoder_multi_Layer import ExplicitRelationEncoder, ImplicitRelationEncoder
...
from mini_mdls.mmdls_relation_encoder_v1 import ImplicitRelationEncoder
...
class MMDLS_model(nn.Module):
    def __init__(self, dataset, w_emb, q_emb, q_att, cap_emb, cap_att, v_relation,
        ...
        super(MMDLS_model, self).__init__()
        ...
        self.joint_proj = FCNet([self.v_relation.q_dim * 2, self.v_relation.q_dim], act='LeakyReLU')
    ...
    def forward(self, v, c, b, q, implicit_pos_emb_L, implicit_pos_emb_R, sem_adj_matrix_L=None,
        ...
        if self.fusion == "ban":
            ...
            joint_emb = torch.cat((v_emb, cap_emb), -1)
            joint_emb = self.joint_proj(joint_emb)
            joint_emb, att = self.joint_embedding(joint_emb, q_emb)
        else:  # mutan
            joint_emb, att = self.joint_embedding(v_emb, q_emb_self_att)
        if self.classifier:
            # v_emb, _ = self.joint_R(v_emb, q_emb)
            # cap_emb, _ = self.joint_L(cap_emb, q_emb)

            v_emb, att_r = self.joint_R(v_emb, q_emb_seq, b)
            cap_emb, att_l = self.joint_L(cap_emb, q_emb_seq, b)
            if self.use_count:
        ...
        out = {'logits': (joint_logits, cap_logits, v_logits), 'q_emb': q_emb_self_att, 'att': (att,att_l,att_r)}
...
def build_mmdls_model(dataset, args):
    ...
    q_emb = TextEmbedding(300 if 'c' not in args.op else 600, args.num_hid, 1, False, .0)
    q_att = TextSelfAttention(args.num_hid, .2)

    cap_emb = TextEmbedding(300 if 'c' not in args.op else 600, args.num_hid, 1, False, .0)
    ...
    total_classifier = SimpleClassifier(args.num_hid, args.num_hid * 2, dataset.num_ans_candidates, 0.5)
    left_classifier = SimpleClassifier(args.num_hid, args.num_hid * 2, dataset.num_ans_candidates, 0.5)
    right_classifier = SimpleClassifier(args.num_hid, args.num_hid * 2, dataset.num_ans_candidates, 0.5)
    gamma = 0
    if args.fusion == "ban":
    ...
    return MMDLS_model(dataset, w_emb, q_emb, q_att, cap_emb, cap_att, v_relation, joint_embedding,
