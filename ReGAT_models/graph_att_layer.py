...
class GraphSelfAttentionLayer(nn.Module):
    ...
    def forward(self, roi_feat, adj_matrix,
        ...
        q_data = self.query(roi_feat)  # U*vi
        ...
        k_data = self.key(nongt_roi_feat)  # U*vj
        ...
        aff = torch.matmul(q_data_batch, torch.transpose(k_data_batch, 2, 3))  # (U*vi).T*(U*vj)
        ...
        if position_embedding is not None and self.pos_emb_dim > 0:
            ...
            position_embedding = position_embedding.float()  # [batch_size, num_rois, nongt_dim, emb_dim]
            # [batch_size,num_rois * nongt_dim, emb_dim]
            position_embedding_reshape = position_embedding.contiguous().view((batch_size, -1, self.pos_emb_dim))
        ...
        if adj_matrix is not None:
            ...
            weighted_aff = torch.transpose(weighted_aff_masked, 2, 3) # [batch_size, N, num_heads, nongt_dim]
        ...
        output_t = torch.matmul(aff_softmax_reshape, v_data) # [batch_size, num_rois*fc_dim, nongt_dim]*[batch_size,nongt_dim, feat_dim]
