...
def torch_extract_position_matrix(bbox, nongt_dim=36):
    ...
    bbox_width = xmax - xmin + 1.  # 每个框的宽
    bbox_height = ymax - ymin + 1.  # 每个框的高
    center_x = 0.5 * (xmin + xmax)  # 每个框的中心x
    center_y = 0.5 * (ymin + ymax)  # 每个框的中心y
    # [batch_size,num_boxes, num_boxes]
    delta_x = center_x-torch.transpose(center_x, 1, 2)  # 没两个框中心点之间的距离x
    ...
    delta_y = center_y-torch.transpose(center_y, 1, 2)  # 没两个框中心点之间的距离y
    delta_y = torch.div(delta_y, bbox_height)
    delta_y = torch.abs(delta_y)
    delta_y[delta_y < threshold] = threshold
    delta_y = torch.log(delta_y)
    delta_width = torch.div(bbox_width, torch.transpose(bbox_width, 1, 2))  # 每两个框的宽之比
