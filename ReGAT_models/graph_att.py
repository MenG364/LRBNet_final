...
from ReGAT_models.graph_att_layer import GraphSelfAttentionLayer
...
class GAttNet(nn.Module):
    ...
    def forward(self, v_feat, adj_matrix, pos_emb=None):
        ...
        Args:
            ...
            pos_emb: [batch_size, num_rois, nongt_dim, emb_dim]
        ...
        nongt_dim = self.nongt_dim  # 20

        adj_matrix = adj_matrix.float()

        adj_matrix_list = [adj_matrix, adj_matrix.transpose(1, 2)]  # [A, A.T]
        ...
        for d in range(self.dir_num):
            ...
            input_adj_matrix = adj_matrix_list[d][:, :, :nongt_dim, :]  # [batch_size,num_rois, nongt_dim,label_num]
            condensed_adj_matrix = torch.sum(input_adj_matrix, dim=-1)  # [batch_size, num_rois, num_rois]

            # [batch_size,num_rois, nongt_dim]
            v_biases_neighbors = self.bias(input_adj_matrix).squeeze(-1) # [batch_size,num_rois, nongt_dim]
