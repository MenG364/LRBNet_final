...
if __name__ == '__main__':
    root='F:/datasets/data/VQA1.0'
    train_answer_file = root+'/Answers/mscoco_train2014_annotations.json'
    train_answers = json.load(open(train_answer_file))['annotations']

    val_answer_file = root+'/Answers/mscoco_val2014_annotations.json'
    val_answers = json.load(open(val_answer_file))['annotations']

    train_question_file = root+'/Questions/OpenEnded_mscoco_train2014_questions.json'
    train_questions = json.load(open(train_question_file))['questions']

    val_question_file = root+'/Questions/OpenEnded_mscoco_val2014_questions.json'
    val_questions = json.load(open(val_question_file))['questions']

    answers = train_answers + val_answers
    occurence = filter_answers(answers, 9)

    cache_path = root+'/cache/trainval_ans2label.pkl'
    if os.path.isfile(cache_path):
        print('found %s' % cache_path)
        ans2label = pickle.load(open(cache_path, 'rb'))
    else:
        ans2label = create_ans2label(occurence, 'trainval',root+'/cache')
    compute_target(train_answers, ans2label, 'train',root+'/cache')
    compute_target(val_answers, ans2label, 'val',root+'/cache')

    # train_answer_file = 'data/cp_annotations/vqacp_train_annotations.json'
    # train_answers = json.load(open(train_answer_file))
    #
    # test_answer_file = 'data/cp_annotations/vqacp_test_annotations.json'
    # test_answers = json.load(open(test_answer_file))
    #
    # train_question_file = 'data/cp_questions/vqacp_train_questions.json'
    # train_questions = json.load(open(train_question_file))
    #
    # test_question_file = 'data/cp_questions/vqacp_test_questions.json'
    ...
    #
    # compute_target(train_answers, ans2label, 'cp_train')
    # compute_target(test_answers, ans2label, 'cp_test')
